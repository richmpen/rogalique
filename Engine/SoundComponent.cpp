...
SoundComponent::SoundComponent(GameObject* gameObject) : Component(gameObject) {
    ...
    LOG_INFO(
        "Created SoundComponent for GameObject: " << gameObject->GetName());
}

SoundComponent::~SoundComponent() { delete sound; }
...
void SoundComponent::SetLoop(bool loop) { sound->setLoop(loop); }
