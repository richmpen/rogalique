...
void GameStateGameWin::Init() {
    ...
    for (int i = 0;
         i < SETTINGS.MAX_RECORDS_TABLE_SIZE && it != sortedRecordsTable.rend();
         ++i, ++it) {
    ...
    if (!isPlayerInTable) {
        ...
        sstream << SETTINGS.MAX_RECORDS_TABLE_SIZE << ". " << SETTINGS.PLAYER_ID
                << ": " << playerScores;
        text.setString(sstream.str());
        text.setFillColor(sf::Color::Green);
    }
...
void GameStateGameWin::Update(float timeDelta) {
    ...
    sf::Color gameWinTextColor =
        (int)timeSinceGameWin % 3 ? sf::Color::Blue : sf::Color::White;
    gameWinText.setFillColor(gameWinTextColor);
    sf::Color scoreTotalTextColor =
        (int)timeSinceGameWin % 2 ? sf::Color::Red : sf::Color::White;
...
void GameStateGameWin::Draw(sf::RenderWindow& window) {
    ...
    DrawTextList(window, textsList, 10.f, Orientation::Vertical, Alignment::Min,
                 tablePosition, {0.5f, 0.f});
