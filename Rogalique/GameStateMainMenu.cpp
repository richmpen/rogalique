...
void GameStateMainMenu::Init() {
    // assert(font.loadFromFile(SETTINGS.RESOURCES_PATH +
    // "Fonts/Alata-Regular.ttf"));
    // assert(backgroundTexture.loadFromFile(SETTINGS.TEXTURES_PATH +
    // "main-menu-Bg.png"));
    InitSprite(background, SETTINGS.SCREEN_WIDTH, SETTINGS.SCREEN_HEIGHT,
               backgroundTexture);
    background.setPosition(SETTINGS.SCREEN_WIDTH / 2,
    ...
    const bool isInfiniteApples =
        Application::Instance().GetGame().IsEnableOptions(
            GameOptions::InfiniteApples);
    MenuItem optionsInfiniteApplesItem;
    optionsInfiniteApplesItem.text.setString(
    ...
    optionsInfiniteApplesItem.onPressCallback = [](MenuItem& item) {
        ...
        bool newOptionValue =
            !game.IsEnableOptions(GameOptions::InfiniteApples);
        game.SetOption(GameOptions::InfiniteApples, newOptionValue);
        item.text.setString("Infinite Apples: " +
                            std::string(newOptionValue ? "On" : "Off"));
    };

    const bool isWithAcceleration =
        Application::Instance().GetGame().IsEnableOptions(
            GameOptions::WithAcceleration);
    MenuItem optionsWithAccelerationItem;
    optionsWithAccelerationItem.text.setString(
    ...
    optionsWithAccelerationItem.onPressCallback = [](MenuItem& item) {
        Game& game = Application::Instance().GetGame();
        bool newOptionValue =
            !game.IsEnableOptions(GameOptions::WithAcceleration);
        game.SetOption(GameOptions::WithAcceleration, newOptionValue);
        item.text.setString("With Acceleration: " +
                            std::string(newOptionValue ? "On" : "Off"));
    };
    ...
    noItem.onPressCallback = [this](MenuItem&) { menu.GoBack(); };
    ...
    scoreTotalText.setString(
        "Total Score: " +
        std::to_string(ScoreSystem::Instance().GetTotalScore()));
