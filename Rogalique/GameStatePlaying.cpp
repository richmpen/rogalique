...
void GameStatePlaying::Init() {
    ...
    assert(bonusEatSoundBuffer.loadFromFile(SETTINGS.SOUNDS_PATH +
    "bonus.wav")); assert(gameOverSoundBuffer.loadFromFile(SETTINGS.SOUNDS_PATH
    + "Death.wav")); assert(gameWinSoundBuffer.loadFromFile(SETTINGS.SOUNDS_PATH
    + "GameWin.wav"));
    assert(backgroundTexture.loadFromFile(SETTINGS.TEXTURES_PATH +
    "BackgroudnGameScene.jpg"));
...
void GameStatePlaying::SavePlayingState() { initialState = CreateMemento(); }

void GameStatePlaying::RestorePlayingState() {}

void GameStatePlaying::Update(float timeDelta) {
    scoreText.setString("Score: " +
                        std::to_string(ScoreSystem::Instance().GetScore()));
    totalScoreText.setString(
        "Total score: " +
