...
Item::Item(ItemType itemType, int itemCount, EngineCore::Vector2Df itemPosition,
...
EngineCore::GameObject* Item::GetGameObject() { return gameObject; }
...
void Item::HideInWorld() {
    if (gameObject) {
        ...
        auto collider =
            gameObject->GetComponent<EngineCore::SpriteColliderComponent>();
        if (collider) {
...
HealthItem::HealthItem(ItemType type, int count,
                       const EngineCore::Vector2Df position,
                       const std::string& textureName)
    : Item(type, count, position, textureName) {
    gameObject =
        EngineCore::GameWorld::Instance()->CreateGameObject("HealthItem");
...
ArmorItem::ArmorItem(ItemType type, int count,
                     const EngineCore::Vector2Df position,
                     const std::string& textureName)
    : Item(type, count, position, textureName) {
    gameObject =
        EngineCore::GameWorld::Instance()->CreateGameObject("ArmorItem");
...
bool ArmorItem::Use(EngineCore::GameObject* player) {
    ...
    if (armorComponent &&
        armorComponent->GetArmor() < armorComponent->GetMaxArmor()) {
...
AmmoItem::AmmoItem(ItemType type, int count,
                   const EngineCore::Vector2Df position,
                   const std::string& textureName)
    : Item(type, count, position, textureName) {
    gameObject =
        EngineCore::GameWorld::Instance()->CreateGameObject("AmmoItem");
...
WeaponItem::WeaponItem(ItemType type, int count,
                       const EngineCore::Vector2Df position,
                       const std::string& textureName)
    : Item(type, count, position, textureName) {
    gameObject =
        EngineCore::GameWorld::Instance()->CreateGameObject("WeaponItem");
