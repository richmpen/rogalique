...
SettingsState::SettingsState() {
    auto uiGameObject =
        EngineCore::GameWorld::Instance()->CreateGameObject("SettingsUI");
...
void SettingsState::CreateUI() {
    ...
    titleText = std::make_shared<TextUI>(
        48, sf::Vector2f(static_cast<float>(SETTINGS.SCREEN_WIDTH) / 2, 100.f),
        true);
    ...
    backButton = std::make_shared<ButtonUI>(
        "Back", "UiMap2", sf::IntRect(1649, 1524, 1216, 182),
        sf::Vector2f(100.f, 100.f), sf::Vector2f(150.f, 40.f), sf::Color::Red);
    backButton->SetCenterOrigin();
    backButton->SetOnClick(
        []() { EngineCore::GameStateManager::Instance()->PopState(); });
    uiManager->AddElement(backButton);

    tabsSystem = std::make_shared<TabsUI>(sf::Vector2f(200.f, 200.f),
...
void SettingsState::CreateGameplayTab() {
    ...
    mazeCheckbox->SetOnValueChanged(
        [this](bool value) { mazeEnabled = value; });
...
void SettingsState::ToggleMusic(bool enable) {
    ...
    for (auto& gameObject : gameObjects) {
        auto soundComponent =
            gameObject->GetComponent<EngineCore::SoundComponent>();
        if (soundComponent) {
            if (enable) {
    ...
    if (!enable) {
        ...
        for (auto& gameObject : gameObjects) {
            if (gameObject->GetComponent<EnemyAIComponent>()) {
                EngineCore::GameWorld::Instance()->DestroyGameObject(
                    gameObject);
            }
        }
    }
