...
namespace Rogalique {
HUDGenerator::HUDGenerator(UiManager* manager, EngineCore::GameObject* gameObject)
...
void HUDGenerator::Generate() {
    if (uiManager) {
        ...
        sf::Vector2f(SETTINGS.PLAYER_CAMERA_WIDTH - 225,
                         SETTINGS.PLAYER_CAMERA_HEIGHT - 87),true);
        ammoTextFirst->SetColor(sf::Color(221, 204, 136));
        ammoTextFirst->SetStyle(sf::Text::Bold);
        

        auto ammoTextSecond = std::make_shared<TextUI>(24,
