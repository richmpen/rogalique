...
namespace Rogalique {
    ...
    class ItemFactory {
    public:
        virtual std::shared_ptr<Item> CreateItem(ItemType type, int count, const EngineCore::Vector2Df& position, const std::string& textureName) = 0;
    };

    class HealthItemFactory final : public ItemFactory {
    public:
        std::shared_ptr<Item> CreateItem(ItemType type, int count, const EngineCore::Vector2Df& position, const std::string& textureName) override;
