...
namespace Rogalique {
HUDGenerator::HUDGenerator(const std::shared_ptr<UiManager>& manager,
                           DeveloperLevel* level, GameObject* gameObject)
    : level(level), gameObject(gameObject), uiManager(manager) {}
...
void HUDGenerator::Generate() {
    if (uiManager) {
        ...
        auto armorIcon = std::make_shared<UiImageElement>(
        ...
        auto ammoIcon = std::make_shared<UiImageElement>(
            "UiMap", sf::IntRect(0, 2382, 235, 235),
